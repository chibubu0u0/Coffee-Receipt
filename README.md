# Coffee Receipt Studio — Firebase v12 Pure Data Receipt

這版是「純資料收據模式」：

- 前台仍保留 receipt 視覺，但不再照 Song Receipt 的情緒分組。
- 只顯示 Firestore / 後台欄位裡實際存在的資料。
- 不自動產生風味百分比、人格、情緒、故事或主觀分析。
...
- 移除風味圖示 chip。
- 官方風味描述會依來源 tasting notes 產生不規則的中心發散色彩融合視覺；這只是來源詞彙的視覺化，不代表酸甜苦等數據，也不會自動評分。


## v12 更新

- 移除 v11 的風味圖示。
- 改成「來源 tasting notes → 色彩視覺」：只有來源或後台欄位裡出現的風味詞才會被用來產生色彩。
- 色彩以不規則、中心向外發散的融合方式呈現，像一張簡約的風味色彩圖。
- 色彩視覺不會轉換成百分比、分數或咖啡感官數據；它只是用來輔助辨識 tasting notes。
